...
function listFiles() {
  try {
    ...
    fetch(
      "http://localhost:58659/api/Documents/FolderFile?id=" +
      ...
      .then((files) => {
        ...
        files.forEach((file) => {
          ...
          console.log(filname);
          console.log(docid);
          fil.innerHTML = `<a><i class="fa fa-file fa-3x" aria-hidden="true" style="color:lightblue;position:absolute;"></i></a><br />
      ...
      <i class="fa fa-trash fa-1.5px" onclick='swalfire(${docid})' style="position:relative;left:130px;bottom:-40px;top:45px;cursor:pointer;color: #618f61;">
      </i>
      <i class='fa fa-star' onclick="favouritefile(${docid})" style="position:relative;left: 130px;bottom:60px;color: #618f61;"></i>`;
...
//function to favorite a file
function favouritefile(documentid) {
  var myHeaders = new Headers();
  myHeaders.append("accept", "*/*");
  ...
  fetch(
    "http://localhost:58659/api/Documents/Filefavourite?id=" + documentid,
    requestOptions
  )
    .then((response) => response.text())
    .then((result) => console.log(result))
